...
class NodeOperationAccessTest extends UnitTestCase {
   ...
   * @param bool|null $shouldCreateNewRevision
   *   Whether the node type has new revisions enabled by default, or NULL if
   *   not to expect it to be called.
   * @param int|null $defaultLanguageRevisionCount
   *   The default language revision count, or NULL if not to expect it to be
   *   called.
  ...
  public function testRevisionOperations($operation, array $hasPermissionMap, $assertAccess, $shouldCreateNewRevision = NULL, $defaultLanguageRevisionCount = NULL, $isDefaultRevision = NULL) {
    ...
    $nodeStorage->expects($this->any())
      ...
      ->with($nid)
      ->willReturn($node);
    if (isset($defaultLanguageRevisionCount)) {
      $nodeStorage->expects($this->atLeastOnce())
    ...
    if (isset($shouldCreateNewRevision)) {
  ...
  public function testRevisionOperationsProvider() {
    ...
    // View all revisions requires shouldCreateNewRevision.
    $data['view all revisions with shouldCreateNewRevision'] = [
      'view all revisions',
      [
        ['access content', TRUE],
    ...
    $data['view all revisions without shouldCreateNewRevision'] = [
      'view all revisions',
      [
        ['access content', TRUE],
        ['view all revisions', TRUE],
    ...
    $data['view all revisions without shouldCreateNewRevision with view access'] = [
      'view all revisions',
      [
        ['access content', TRUE],
        ['view all revisions', TRUE],
    ...
    // Can never delete default revision.
    $data['delete revision, with delete access, one revision, default revision'] = [
      'delete revision',
      [
        ['access content', TRUE],
        ['delete all revisions', TRUE],
    ...
    $data['delete revision, with delete access, more than one revision, default revision'] = [
      'delete revision',
      [
        ['access content', TRUE],
        ['delete all revisions', TRUE],
